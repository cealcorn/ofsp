...
public class Multithreadserver2 extends Thread {
    ...
    private static class handler extends Thread {
                        ...
                        switch (command) {
                            ...
                            case 'M':
                                ...
                                try {
                                    ...
                                    System.out.println(
                                            "Could not rename/delete folder because the requested file was a directory");
                                }
                                break;
                            case 'H':
                                humanReadableCommand = "Help";
                            ...
                            case 'T':
                                ...
                                try {
                                    ...
                                    System.out.println(
                                            "Error from client delete command while deleting: " + e.getMessage());
                                }
                                break;
                            default:
                                throwError(
        ...
        private static File checkFileAvailability(String name)
            ...
            if (!file.exists()) {
                ...
                throw new handler.IncorrectFileNameException(IncorrectFileNameException.BadFileName, name);
            }
            if (file.isDirectory()) {
                throwError("file is a directory");
                throw new handler.IsDirectoryException("The file specified is a directory");
            }
            if (file.canRead()) {
                return file;
            } else {
                throwError("Server error occured, contact server admin if this keeps occuring; CODE: READ DENIED");
                throw new handler.BadPermissionsException(BadPermissionsException.BadRead);
            }
        }
        ...
        private static File checkFolderAvailability(String path) throws IsFileException, IncorrectDirectoryName {
            ...
            if (directory.isFile()) {
                ...
                throw new handler.IsFileException("Directory is shown as a file");
            }
            if (directory.exists()) {
                return directory;
            } else {
                throwError("unknown directory: " + path);
                throw new handler.IncorrectDirectoryName("Unknown Directory");
            }
        }
        ...
        private static void deleteFile(File file) throws FileDeletionException {
            if (!file.delete()) {
                throw new handler.FileDeletionException("Unable to delete file");
            }
        }

        private static void moveFile(File file, String destination, boolean rename)
                throws BadPermissionsException, IsDirectoryException {
            String command;
            if (rename) {
                command = "rename";
            } else {
        ...
        // because new lines break the code, this is a sanitazation function th=o make
        // sure that the code does not break
    ...
    Multithreadserver2(Socket passedClientSocket) {
        thisServer = this;
    ...
    private static int verify_arguments(String[] args) throws InvalidPortNumber, InvalidArgumentLength {
        if (args.length != 1) {
            throw new Multithreadserver2.InvalidArgumentLength("Invalid number of arguments");
        }
        ...
        try {
            ...
            throw new Multithreadserver2.InvalidPortNumber("Port not an integer");
        }

        if (port > 65535) {
            System.out.println("The port is above the range, please input a port between 1 and 65535");
            throw new Multithreadserver2.InvalidPortNumber("Port above range.");
        } else if (port < 1) {
            System.out.println("The port is below the range, please input a port between 1 and 65535");
            throw new Multithreadserver2.InvalidPortNumber("Port below range.");
        }
        return port;
    }
    ...
    public static void main(String[] args) {
        ...
        // get the seperator here because if you dont have server conection, this is
        // pointless (you also only need to do this once)
        // getDirectorySeperator();

        while (true) { // keeps server connection open and accepts multiple clients
            try {
                // Socket clientSock = serveSocket.accept();
                new Multithreadserver2.handler(serveSocket.accept()).start();
            } catch (IOException e) {
